...
public class GoHomeAndSleepTilRested : State<Miner>
{
    ...
    public override void Enter(Miner miner)
    {
        ...
        // 让妻子知道我回家了
        MessageDispatcher.Instance.DispatchMessage(0, miner.ID, 2, MessageType.HiHoneyImHome, null);
    ...
    public override bool OnMessage(Miner miner, Telegram message)
    {
        switch (message.Message)
        {
            case MessageType.StewReady:
                Debug.Log(string.Format("消息已被 {0} 处理，时间为：{1}", miner.Name, Time.time));
                Debug.Log(string.Format("{0}：好~~我来吃饭啦！", miner.Name));
...
{
    public static EatStew Instance { get; private set; }
    ...
    public override void Enter(Miner miner)
    {
        Debug.Log(string.Format("{0}：闻起来好~香~！！！", miner.Name));
    }

    public override void Execute(Miner miner)
    {
        Debug.Log(string.Format("{0}：吃起来也好~香！！！", miner.Name));
    ...
    {
        Debug.Log(
            string.Format("{0}：爱你老婆！比心！是时候回去继续做事啦！", miner.Name));
    }
